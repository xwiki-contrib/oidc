 ...
 */
@Component
@Named(OIDCProviderStoreListener.NAME)
@Singleton
public class OIDCProviderStoreListener extends AbstractEventListener
{
    /**
    ...
    @Inject
    private Provider<OIDCProviderStore> storeProvider;

    /**
     * The default constructor.
     */
    public OIDCProviderStoreListener()
    {
    ...
    public void onEvent(Event event, Object source, Object data)
    {
        if (event instanceof AuthorizationCodeCreatedEvent) {
            this.storeProvider.get().getAuthorizationSessionMap().put(((AuthorizationCodeCreatedEvent) event).getCode(),
                (AuthorizationSession) source);
        } else if (event instanceof AuthorizationCodeDeletedEvent) {
            this.storeProvider.get().getAuthorizationSessionMap()
                .remove(((AuthorizationCodeDeletedEvent) event).getCode());
        }
    }
}
